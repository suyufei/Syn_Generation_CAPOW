...
for i in range(0,sim_days):
    ...
    generation[i,8] = ((1000*19.812*pf_trans*9.81*.86)/1000000)*24 
    ...
    storage[i,23] = s_24 
    generation[i,23] = ((1000*19.812*powerflow[i,23]*14.27641*9.81*.73)/1000000)*24 
    ...
    storage[i,24] = s_25 
    generation[i,24] = ((1000*23.1648*powerflow[i,24]*14.27641*9.81*.82)/1000000)*24 
    ...
    storage[i,25] = s_26 
    generation[i,25] = ((1000*20.42*powerflow[i,25]*14.27641*9.81*.71)/1000000)*24 
    ...
    storage[i,26] = s_26 
    generation[i,26] = ((1000*53.34*powerflow[i,26]*14.26641*9.81*.76)/1000000)*24 
    ...
    storage[i,27] = s_27  
    generation[i,27] = ((1000*19.812*powerflow[i,27]*14.27641*9.81*.8)/1000000)*24 
    ...
    RVC_dg = MCD_dg + local.iloc[i,15]*.001987 
...
#        VARQnew_LIB = np.zeros((sim_days,1))  
    ...
    generation[i,2] = ((1000*LIB_head[i]*powerflow[i,2]*14.27641*9.81*.865)/1000000)*24 
    
    ######################################
    #Bonner's Ferry - run-of-river project
    ######################################

    BFE_dg = LIB_dg + local.iloc[i,46]*.001987 
    ...
    storage[i,46] = s_47  
    generation[i,46] = max(0,((1000*28.0416*powerflow[i,46]*14.27641*9.81*.875)/1000000)*24) 

    ######################################
    #Corra Linn - run-of-river project
    ######################################

    CLN_dg = BFE_dg + DNC_dg + local.iloc[i,4]*.001987 
    ...
    storage[i,4] = s_5  
    generation[i,4] = max(0,((1000*16.1544*powerflow[i,4]*14.27641*9.81*.875)/1000000)*24) 

    ######################################
    #Brilliant - run-of-river project
    ######################################

    BRI_dg = CLN_dg + local.iloc[i,17]*.001987 
    ...
    storage[i,17] = s_18  
    generation[i,17] = max(0,((1000*28.0416*powerflow[i,17]*14.27641*9.81*.875)/1000000)*24) 
...
#        VAQRnew_HHO = np.zeros((sim_days,1))
    ...
    if i > 0:
        KER_dg = discharge[i-1,5]*.001987 + + CFM5L.iloc[i]*.001987+ local.iloc[i,6]*.001987 
        s_7 = s_7 + discharge[i-1,5]*.001987 + local.iloc[i,6]*.001987 + CFM5L.iloc[i]*.001987 - KER_dg 
    else:
        KER_dg = HHO_dg + local.iloc[i,6]*.001987+ CFM5L.iloc[i]*.001987 
    ...
    THF_dg = KER_dg + local.iloc[i,18]*.001987 
    ...
    storage[i,18] = s_19  
    generation[i,18] = max(0,((1000*19.5072*powerflow[i,18]*14.27641*9.81*.875)/1000000)*24) 
    ...
    if i > 0:
        NOX_dg = discharge[i-1,18]*.001987 + local.iloc[i,13]*.001987 
        s_14 = s_14 + discharge[i-1,18]*.001987 + local.iloc[i,13]*.001987 - NOX_dg 
    else:
        NOX_dg = THF_dg + local.iloc[i,13]*.001987 
    ...
    CBG_dg = NOX_dg + local.iloc[i,19]*.001987 
    ...
    storage[i,19] = s_19     
    generation[i,19] = max(0,((1000*33.8328*powerflow[i,19]*14.27641*9.81*.875)/1000000)*24) 
    
    ######################################
    #Priest Lake - run-of-river project
    ######################################

    PSL_dg = local.iloc[i,45]*.001987 
...
#    ALB_sfore = np.zeros((sim_days,1))
    ...
    BOX_dg = local.iloc[i,20]*.001987 + ALB_dg 
    ...
    storage[i,20] = s_21   
    generation[i,20] = max(0,((1000*14.0208*powerflow[i,20]*14.27641*9.81*.875)/1000000)*24) 
    
    ######################################
    #Boundary - run-of-river project
    ######################################

    BOU_dg = local.iloc[i,21]*.001987 + BOX_dg 
    ...
    storage[i,21] = s_22 
    generation[i,21] = max(0,((1000*103.632*powerflow[i,21]*14.27641*9.81*.875)/1000000)*24) 
    
    ######################################
    #Seven Mile - run-of-river project
    ######################################

    SVN_dg = local.iloc[i,16]*.001987 + BOU_dg 
    ...
    storage[i,16] = s_17 
    generation[i,16] = max(0,((1000*64.92*powerflow[i,16]*14.27641*9.81*.875)/1000000)*24) 
    
    ######################################
    #Waneta - run-of-river project
    ######################################

    WNT_dg = local.iloc[i,22]*.001987 + SVN_dg 
    ...
    storage[i,22] = s_23 
    generation[i,22] = max(0,((1000*23.1648*powerflow[i,22]*14.27641*9.81*.875)/1000000)*24) 
...
#    Spillheads = np.zeros((sim_days,11))
    ...
    CHJ_dg = local.iloc[i,28]*.001987 + GCL_dg 
    ...
    WLL_dg = local.iloc[i,29]*.001987 + CHJ_dg 
    ...
    RRH_dg = local.iloc[i,30]*.001987 + CHL_dg + WLL_dg 
    ...
    storage[i,30] = s_31 
    generation[i,30] = max(0,((1000*24.3108*powerflow[i,30]*14.27641*9.81*.873)/1000000)*24) 
    
    #########################
    #Rock Island - run-of-river
    #########################
    
    RIS_dg = local.iloc[i,31]*.001987 + RRH_dg 
    ...
    storage[i,31] = s_32 
    generation[i,31] = max(0,((1000*11.5640*powerflow[i,31]*14.27641*9.81*.874)/1000000)*24) 
    
    #########################
    #Wanapum - run-of-river
    #########################
    
    WPM_dg = local.iloc[i,32]*.001987 + RIS_dg 
    ...
    storage[i,32] = s_33 
    generation[i,32] = max(0,((1000*19.2734*powerflow[i,32]*14.27641*9.81*.873)/1000000)*24) 
    
    #########################
    #Priest Rapids - run-of-river
    #########################
    
    PRD_dg = local.iloc[i,33]*.001987 + WPM_dg 
    ...
    storage[i,33] = s_34 
    generation[i,33] = max(0,((1000*17.698*powerflow[i,33]*14.27641*9.81*.873)/1000000)*24) 
    ...
    OXB_dg = local.iloc[i,34]*.001987 + BRN_dg 
    ...
    HLC_dg = local.iloc[i,43]*.001987 + OXB_dg 
    ...
    storage[i,43] = s_44 
    generation[i,43] = max(0,((1000*53.62*powerflow[i,43]*14.27641*9.81*.875)/1000000)*24) 
    ...
    #and Lower Granite. 
    if i>0:
        LWG_dg = local.iloc[i,35]*.001987 + discharge[i-1,12]*.001987 + discharge[i-1,43]*.001987 + ORF5H[i]* .001987 + SPD5L[i]*.001987 + ANA5L[i]*.001987 + LIM5L[i]*.001987 + WHB5H[i]*.001987 
        ...
        LWG_dg = local.iloc[i,35]*.001987 + discharge[i,12]*.001987 + discharge[i,43]*.001987+ ORF5H[i]* .001987 + SPD5L[i]*.001987 + ANA5L[i]*.001987 + LIM5L[i]*.001987 + WHB5H[i]*.001987 
    ...
    #Calculate daily reservoir discharge and end-of-day storage. Have to
    #convert project inflows ("local") to kAF.
    LGO_dg = local.iloc[i,36]*.001987 + LWG_dg 
    ...
    #Calculate daily reservoir discharge and end-of-day storage. Have to
    #convert project inflows ("local") to kAF.
    LMN_dg = local.iloc[i,37]*.001987 + LGO_dg 
    ...
    generation[i,37] = ((1000*LMN_head[i]*powerflow[i,37]*14.27641*9.81*.83)/1000000)*24 
    ...
    #Calculate daily reservoir discharge and end-of-day storage. Have to
    #convert project inflows ("local") to kAF.
    IHR_dg = local.iloc[i,38]*.001987 + LMN_dg 
    ...
    #Calculate daily reservoir discharge and end-of-day storage. Have to
    #convert project inflows ("local") to kAF.
    PLT_dg = local.iloc[i,44]*.001987 + RBU_dg 
    ...
    #Calculate daily reservoir discharge and end-of-day storage. Have to
    #convert project inflows ("local") to kAF.
    MCN_dg = local.iloc[i,39]*.001987 + PRD_dg +IHR_dg + YAK5H[i]*.001987 
    ...
    generation[i,39] = max(0,((1000*MCN_head[i]*powerflow[i,39]*14.27641*9.81*.76)/1000000)*24) 
    ...
    #Calculate daily reservoir discharge and end-of-day storage. Have to
    #convert project inflows ("local") to kAF.
    JDA_dg = local.iloc[i,40]*.001987 + MCN_dg 
    ...
    #Calculate daily reservoir discharge and end-of-day storage. Have to
    #convert project inflows ("local") to kAF.
    TDA_dg = local.iloc[i,41]*.001987 + JDA_dg + PLT_dg 
    ...
    #Calculate daily reservoir discharge and end-of-day storage. Have to
    #convert project inflows ("local") to kAF.
    BNN_dg = local.iloc[i,42]*.001987 + TDA_dg 
